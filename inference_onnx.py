...
def process_one_image(dir_path, img_name, transform, session):#此函数默认batch_size为1
    ...
    img_raw = img_raw.resize((128, 512), Image.LANCZOS)
    # pre-proccessing
...
def main(args):
    ...
    img_paths = [i for i in os.listdir(dir_path) if 'bmp' in i]
...
def get_args_parser():
    ...
    parser.add_argument('--input_dir', default=r'D:\dataset\计数\crop',
                        help='path where to read images')
    parser.add_argument('--output_dir', default=r'D:\code\CrowdCounting-P2PNet\result_onnx',
                        help='path where to save')
    parser.add_argument('--weight_path', default=r'D:\dataset\计数\crop\计数检测2024-0910-183429\model.onnx',
